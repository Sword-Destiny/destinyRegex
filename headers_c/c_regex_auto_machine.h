...
void s_condition_add_s_condition(struct list_char* list) {
	if (!current_s_condition) {
		...
		temp_s_condition = current_s_condition->next;
	}
}
...
bool change_letter(struct regex* reg, int begin, int end) {
	...
	switch (reg->str[begin]) {
		...
		case NOT_SIMPLE_LETTER:
			...
			char_list_add_many_char('Z' + 1, 'a' - 1);
			char_list_add_many_char('z' + 1, 126);
			char_list_add_char(127);
			return true;
		case RANGE_LETTER:
...
bool get_match_num(struct regex* reg, int begin, int end) {
	if (begin > end || reg->str[begin] != LEFT_LARGE_BRACE) {
		if (reg->str[begin] == PLUS) {
			temp_index = begin;
			temp_least_num = 1;
			temp_max_num = MAX_INT_VAL;
			return true;
		}
		if (reg->str[begin] == MULTI_MATCH_LETTER) {
			temp_index = begin;
			temp_least_num = 0;
			temp_max_num = MAX_INT_VAL;
			return true;
	...
	for (int i = begin + 1; i <= end; i++) {
		if (reg->str[i] == BLANK || reg->str[i] >= '0'&&reg->str[i] <= '9') {
			if (temp_num > 9) {
				error_str = "匹配次数的数字太大!";
				free(buffer);
				return false;
			}
			buffer[temp_num] = reg->str[i];
			temp_num++;
		}
		else if (reg->str[i] == COMMA) {
			if (comma_num > 0) {
				error_str = "匹配次数语法错误!";
				free(buffer);
				return false;
		...
		else if (reg->str[i] == RIGHT_LARGE_BRACE) {
			if (comma_num > 0) {
				buffer[temp_num] = '\0';
				if (is_num(buffer)) {
